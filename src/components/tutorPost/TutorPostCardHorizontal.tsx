...
const TutorPostCardHorizontal: React.FC<TutorPostCardHorizontalProps> = ({ 
  ...
  const getCompatibilityColor = (score: number): string => {
    ...
    if (percentage >= 90) return "bg-gradient-to-r from-green-50 to-emerald-50 text-green-800 border-green-300";
    if (percentage >= 80) return "bg-gradient-to-r from-blue-50 to-cyan-50 text-blue-800 border-blue-300";
    if (percentage >= 70) return "bg-gradient-to-r from-yellow-50 to-amber-50 text-yellow-800 border-yellow-300";
    if (percentage >= 60) return "bg-gradient-to-r from-orange-50 to-red-50 text-orange-800 border-orange-300";
  ...
  return (
    <motion.div
      ...
      className={`
        ...
        ${showCompatibility && post.compatibility && getCompatibilityPercentage(post.compatibility) >= 80
    ...
    >
      ...
      <div className="p-5 flex items-center gap-5">
        ...
        <div className="flex-1 min-w-0 space-y-2.5">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            {showCompatibility && post.compatibility !== undefined && (
              <div className={`flex items-center px-2.5 py-1.5 rounded-lg text-xs font-bold border ${getCompatibilityColor(post.compatibility)} flex-shrink-0`}>
                <span className="mr-1.5">{getCompatibilityIcon(post.compatibility)}</span>
                <span>{getCompatibilityText(post.compatibility)}: {getCompatibilityPercentage(post.compatibility)}%</span>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            {post.tutorId.gender && post.tutorId.date_of_birth && <span>•</span>}
            {post.tutorId.gender && (
              <span>
                {post.tutorId.gender === "male" ? "Nam" : post.tutorId.gender === "female" ? "Nữ" : "Khác"}
              </span>
            )}
            <span>•</span>
          ...
          <p className="text-gray-600 text-sm line-clamp-1 leading-relaxed">
            {truncateText(post.tutorId.profile?.introduction || post.description, 80)}
          </p>
        ...
        <div className="flex-shrink-0 flex flex-col items-end gap-3 min-w-0">
          ...
          {/* Teaching Mode Badge */}
          <div className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${modeConfig.color} flex items-center flex-shrink-0`}>
          ...
          <div className="text-right space-y-1.5">
            <div className="bg-gradient-to-r from-green-500 to-emerald-500 text-white px-3 py-1.5 rounded-lg min-w-0">
              <div className="flex items-center text-sm font-bold">
                ...
                <span className="truncate">{formatPrice(post.pricePerSession)}</span>
              </div>
            </div>
          ...
          <button
            ...
            `}
          >
            {showCompatibility && post.compatibility && getCompatibilityPercentage(post.compatibility) >= 90 
              ? '🎯 Liên hệ ngay' 
              : showCompatibility && post.compatibility && getCompatibilityPercentage(post.compatibility) >= 80
              ? '✨ Xem chi tiết'
              : 'Xem chi tiết'
          ...
          <div className="pt-2.5 grid grid-cols-2 md:grid-cols-4 gap-3">
            {post.matchDetails.subjectMatch > 0 && (
              <div className="flex items-center text-xs text-gray-600">
                ...
                <span className="truncate">Môn: {Math.round(post.matchDetails.subjectMatch)}%</span>
              </div>
            )}
            {post.matchDetails.locationMatch > 0 && (
              <div className="flex items-center text-xs text-gray-600">
                <MapPinIcon className="w-3 h-3 mr-1 text-red-500 flex-shrink-0" />
                <span className="truncate">Vị trí: {Math.round(post.matchDetails.locationMatch)}%</span>
              </div>
            )}
            {post.matchDetails.priceMatch > 0 && (
              <div className="flex items-center text-xs text-gray-600">
                <CurrencyDollarIcon className="w-3 h-3 mr-1 text-green-500 flex-shrink-0" />
                <span className="truncate">Giá: {Math.round(post.matchDetails.priceMatch)}%</span>
              </div>
            )}
            {post.matchDetails.scheduleMatch > 0 && (
              <div className="flex items-center text-xs text-gray-600">
                <ClockIcon className="w-3 h-3 mr-1 text-purple-500 flex-shrink-0" />
                <span className="truncate">Lịch: {Math.round(post.matchDetails.scheduleMatch)}%</span>
              </div>
            )}
          </div>
        </motion.div>
