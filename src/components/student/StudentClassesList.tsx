...
import {
  ...
  XCircleIcon,
  BookmarkSlashIcon, // Icon cho trạng thái rỗng
...
const StudentClassesList: React.FC = () => {
  ...
  // --- Empty State (Làm đẹp hơn) ---
  if (!studentClasses || studentClasses.length === 0) {
    return (
      <div className="bg-white shadow-lg rounded-lg p-8 text-center">
        <BookmarkSlashIcon className="h-16 w-16 text-gray-400 mx-auto mb-4" />
        <h3 className="text-xl font-semibold text-gray-800 mb-2">Chưa có lớp học nào</h3>
        <p className="text-gray-500">Bạn chưa đăng ký tham gia lớp học nào.</p>
      </div>
    );
  }
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {studentClasses.map((classItem) => (
              <tr key={classItem._id} className="hover:bg-gray-50 transition-colors">
                ...
                {/* Cột Lớp học (Gộp Tên Lớp + Môn học) */}
                ...
                {/* Cột Gia sư */}
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm text-gray-900">{classItem.tutorId?.full_name}</div>
                </td>
            ...
            <div key={classItem._id} className="bg-white shadow rounded-lg p-4">
              ...
              <div className="space-y-2 mb-4 border-t pt-3">
                <div className="flex items-center gap-2 text-sm text-gray-700">
                  ...
                  <span>{classItem.tutorId?.full_name}</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-gray-700">
