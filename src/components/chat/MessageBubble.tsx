    ...
    if (message.messageType === "file" && message.fileMetadata?.fileUrl) {
      ...
      return (
        <div className="space-y-2">
          ...
          >
            <div
              className={`flex-shrink-0 text-2xl ${isOwn ? "opacity-90" : ""}`}
            >
          ...
          {message.content &&
            message.content !== message.fileMetadata.fileName && (
    ...
    <>
      <div className={`flex w-full ${bubbleAlignment}`}>
        <div className="max-w-xs md:max-w-md">
          <div
            className={`px-4 py-2 rounded-2xl ${bubbleRadius} ${bubbleClasses}`}
          >
            {renderContent()}
          </div>
          <div
            className={`flex items-center mt-1 space-x-1 ${bubbleAlignment}`}
          >
        ...
        message.fileMetadata?.fileUrl && (
          ...
          >
            <div className="relative max-w-7xl max-h-full">
              <button
                onClick={() => setShowImageModal(false)}
                className="absolute -top-12 right-0 cursor-pointer text-white hover:text-gray-300 transition-colors"
              >
                <X size={32} />
              </button>
              <img
