...
const HomeworkModal: React.FC<HomeworkModalProps> = ({
  ...
  const transcribeAudioToText = async (audioUrl: string) => {
    ...
    try {
      ...
      const transcript = response.data?.transcript || response.data?.data?.transcript || '';
      if (transcript) {
        setSubmissionData(prev => ({ ...prev, textAnswer: transcript }));
        toast.success('Đã chuyển audio thành văn bản, vui lòng kiểm tra lại trước khi nộp.');
      } else {
  ...
  const handleFileUpload = async (
    ...
    if (target === 'submissionAudio') {
      const allowedExt = ['flac', 'm4a', 'mp3', 'mp4', 'mpeg', 'mpga', 'oga', 'ogg', 'wav', 'webm'];
                              ...
                              <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-sm">
                                ...
                                <div className="mt-2 space-y-2">
                                  ...
                                  {assignment.submission.textAnswer && (
                                    <div>
                                      <span className="font-medium text-gray-700">Bài viết:</span>
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setAssignmentData((prev) => ({ ...prev, deadline: e.target.value }))
                    }
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  />
                </div>
              ...
              >
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                  <p className="text-sm text-yellow-800">
                    <strong>Lưu ý:</strong> Hãy kiểm tra kỹ bài làm trước khi nộp. Nếu bạn đã nộp trước đó,
                    ...
                    {canSubmitAssignments.map((assignment, index) => {
                      ...
                      return (
                        <option key={assignment.id || `pending-${index}`} value={assignment.id}>
                          {assignment.title || `Bài tập ${index + 1}`}{status}
                        </option>
                      );
                    })}
                  </select>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Bài nói (audio) <span className="text-xs text-gray-500">(tùy chọn, cho speaking)</span>
                  </label>
                  <div className="flex items-center space-x-3">
                    <label className="flex-1">
                      <input
                      ...
                      <div className="w-full px-4 py-2 border-2 border-dashed border-gray-300 rounded-lg hover:border-purple-400 cursor-pointer transition-colors flex items-center justify-center space-x-2 text-gray-600 hover:text-purple-600">
                        <CloudArrowUpIcon className="w-5 h-5" />
                        <span>{uploadingFile ? 'Đang tải...' : 'Chọn file audio'}</span>
                      </div>
                    </label>
                  </div>
                  <p className="mt-1 text-xs text-gray-500">
                    Hệ thống sẽ dùng dịch vụ Speech-to-Text để chuyển giọng nói thành văn bản (sử dụng cho AI chấm điểm).
                  </p>
                  {transcribingAudio && (
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setSubmissionData((prev) => ({ ...prev, textAnswer: e.target.value }))
                    }
                    rows={4}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-6">
                {loadingLibrary ? (
                  ...
                  <div className="text-center py-12">
                    <DocumentTextIcon className="w-16 h-16 text-gray-300 mx-auto mb-4" />
                    <p className="text-gray-500 mb-4">Kho bài tập của bạn đang trống</p>
                    <button
                      onClick={() => {
                        setShowTemplatePicker(false);
                        navigate('/tutor/exercise-bank');
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-gray-600 line-clamp-2 mb-2">
                                {template.description || template.content?.prompt || 'Không có mô tả'}
                              </p>
                              <div className="flex flex-wrap gap-2 items-center">
                                {template.subjectId && (
                                ...
                                {template.difficulty && (
                                  <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs border ${
                                    template.difficulty === 'EASY' ? 'bg-green-50 text-green-700 border-green-200' :
                                    template.difficulty === 'MEDIUM' ? 'bg-yellow-50 text-yellow-700 border-yellow-200' :
