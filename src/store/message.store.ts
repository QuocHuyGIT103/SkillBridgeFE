...
import type { MessageData, ConversationData, SendMessageRequest } from '../services/message.service';
import { socketService } from '../services/socket.service';
import toast from 'react-hot-toast';
import { normalizeMessage, normalizeConversation } from '../services/message.service';
...
interface MessageState {
  ...
  sendMessage: (conversationId: string, data: SendMessageRequest) => Promise<void>;
  createConversation: (contactRequestId: string) => Promise<ConversationData | null>;
...
const useMessageStore = create<MessageState>((set, get) => ({
  ...
  createConversation: async (contactRequestId: string) => {
    try {
      const conversation = await messageService.createConversation({ contactRequestId });
  ...
  getConversationByContactRequest: async (contactRequestId: string) => {
    try {
      const conversation = await messageService.getConversationByContactRequest(contactRequestId);
      return conversation;
    } catch (error: any) {
      set({ error: error.message });
      return null;
  ...
  handleNewMessage: (rawMessage: any) => {
    ...
    const alreadyExists = currentMessages.some(m => String(m._id) === String(message._id));

    // Add message if it belongs to the current conversation and not already in list
    if (selectedConversation && message.conversationId === selectedConversation._id && !alreadyExists) {
      ...
      if (conv._id === message.conversationId) {
        return {
          ...
          unreadCount: {
            ...
            student: message.receiverId === conv.studentId._id
              ? conv.unreadCount.student + 1
              : conv.unreadCount.student,
            tutor: message.receiverId === conv.tutorId._id
