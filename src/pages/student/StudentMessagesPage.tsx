...
const StudentMessagesPage: React.FC = () => {
  ...
  const totalMessages = conversations.reduce((sum, c) => {
    // messageCount may not exist, default to 0
    const msgCount = (c as any).messageCount;
    if (typeof msgCount === "object") {
      return sum + ((msgCount.student || 0) + (msgCount.tutor || 0));
    }
    return sum + (msgCount || 0);
  }, 0);
