...
const StudentRequestCard: React.FC<StudentRequestCardProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`absolute inset-0 bg-gradient-to-br ${style.gradient} opacity-70`}
      />
      <div
      ...
      <div className="relative p-6 md:p-7 space-y-6">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            >
              {React.cloneElement(getStatusIcon(request.status ?? "PENDING"), {
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {request.initiatedBy === "TUTOR" ? "Nhận lúc" : "Gửi lúc"}{" "}
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-3 self-start">
            ...
            {request.initiatedBy === "TUTOR" && request.status === "PENDING" ? (
              ...
              <>
                <Link
                  to={
                    requestId ? `/student/contact-requests/${requestId}` : "#"
                  }
                ...
                {canCancel && request.initiatedBy !== "TUTOR" && (
                  <button
                    onClick={(e) => {
                      ...
                      e.stopPropagation();
                      console.log(
                        "Cancel button clicked, requestId:",
                        requestId
                      );
                      if (requestId) {
                        onCancel(requestId);
        ...
        <div>
          ...
          <div className="relative rounded-2xl border border-blue-100 bg-blue-50/70 p-4">
            <span className="absolute -top-4 left-4 text-4xl text-blue-200">
              “
            </span>
            <p className="text-sm text-blue-900 leading-6 pl-4 pr-2">
              {request.message}
            </p>
            <span className="absolute -bottom-6 right-6 text-4xl text-blue-200">
              ”
            </span>
          </div>
        </div>
        ...
        {request.status === "ACCEPTED" && (
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50/80 p-4 text-sm text-emerald-800">
            🎉 Gia sư đã chấp nhận yêu cầu của bạn! Hãy kiểm tra email hoặc điện
            thoại để cập nhật lịch học.
          </div>
        )}

        {isExpired && (
          <div className="rounded-2xl border border-amber-200 bg-amber-50/80 p-4 text-sm text-amber-800">
            ⏰ Yêu cầu đã hết hạn vào {formatDate(request.expiresAt!)}. Bạn có
            thể gửi yêu cầu mới để tiếp tục tìm gia sư.
          </div>
        )}
      </div>
    </motion.div>
...
const StudentResponseButtons: React.FC<StudentResponseButtonsProps> = ({
  requestId,
}) => {
  const { studentRespondToRequest, isResponding, getStudentRequests } =
    useContactRequestStore();
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6">
            ...
            <div className="mb-4">
              ...
              <textarea
                ...
                placeholder={
                  showConfirmModal === "ACCEPT"
                    ? "Cảm ơn gia sư, tôi rất vui được học với bạn..."
                    : "Xin lỗi, tôi không thể chấp nhận đề nghị này..."
                }
              />
            </div>
