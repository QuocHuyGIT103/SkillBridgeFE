...
const CreateTeachRequestPage: React.FC = () => {
  ...
  const tutorPostIdFromState =
    location?.state?.tutorPostId || location?.state?.selectedTutorPostId || "";
  const isFromAIRecommendations =
    location?.state?.fromAIRecommendations || false;

  const [tutorPostId, setTutorPostId] = useState<string>(tutorPostIdFromState);
  const [subject, setSubject] = useState<string>("");
  const [message, setMessage] = useState<string>(
    "Xin chào, tôi tin rằng mình phù hợp để dạy bạn. Hãy cùng trao đổi thêm nhé!"
  );
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!finalTutorPostId) {
      const subjectName =
        activeSubjects?.find((s) => s._id === subject)?.name || "";
      ...
      if (matchingPost) {
        ...
        toast.error(
          "Không tìm thấy bài đăng có môn học này. Vui lòng chọn bài đăng hoặc tạo bài đăng mới."
        );
        return;
      }
    }
          ...
          <div className="md:col-span-3">
            <form onSubmit={onSubmit} className="space-y-4">
                ...
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  ...
                  {!isFromAIRecommendations && (
                    <span className="ml-2 text-xs text-gray-500 font-normal">
                      (Tùy chọn - sẽ tự động chọn bài đăng có môn học đã chọn)
                    </span>
                  )}
                </label>
                <select
              ...
              <div>
                ...
                >
                  <option value="">
                    {loadingMyPosts ? "Đang tải..." : "— Chọn môn dạy —"}
                  </option>
                  ...
                  {selectedPost.hourly_rate && (
                    <div>
                      ...
                      <strong>
                        ...
                        - {selectedPost.hourly_rate.max.toLocaleString("vi-VN")}
                        đ
                      </strong>
                    </div>
                  )}
