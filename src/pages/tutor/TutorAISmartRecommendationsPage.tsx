...
const TutorAISmartRecommendationsPage: React.FC = () => {
  ...
  const fetchRecommendations = async () => {
    ...
    try {
      ...
      if (tutorPostId) {
        ...
        response = await PostService.smartSearchStudentPostsForTutor(
          smartQuery
        );

        if (response.success && response.data) {
          // Helper function removed - detailed explanation can be generated if needed
          ...
          let transformedRecommendations = await Promise.all(
            response.data.posts.map(async (post: any) => {
              ...
              // Student post data construction removed
              // const _unused_studentPostData = {
              ...
              return {
                ...
                studentPost: {
                  ...
                  author: {
                    name:
                      post.authorId?.full_name ||
                      post.author?.full_name ||
                      "Học viên",
                    email: post.authorId?.email || post.author?.email || "",
                    phone:
                      post.authorId?.phone_number || post.author?.phone_number,
                    avatar:
                      post.authorId?.avatar_url || post.author?.avatar_url,
                  },
                },
                matchDetails: {
          ...
          // Apply limit
          transformedRecommendations = transformedRecommendations.slice(
            0,
            query.limit || 10
          );
        ...
        response = await AIService.getSmartStudentRecommendations(
          user.id,
          query
        );
        ...
        if (response.success && response.data) {
          ...
          const totalCount =
            response.data.total || response.data.recommendations.length;
  ...
  // AI Status Banner component removed - not used in current version
  // const _unused_AIStatusBanner = () => {...};
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center py-20">
            ...
            <p className="text-gray-600">
              Đang tìm kiếm các bài đăng tìm gia sư phù hợp nhất với khả năng
              dạy của bạn
            </p>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <select
                ...
                onChange={(e) =>
                  handleQueryChange({ minScore: Number(e.target.value) })
                }
                className="px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:ring-2 
                          focus:ring-blue-500 focus:border-transparent"
              >
